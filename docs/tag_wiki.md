...
2. **parse** —— 解析 DText：剥离标记语法、按标题切分章节（超过 1200 字符按段落续切）、提取 `[[wiki 链接]]`/`{{tag}}` 生成关联表；按 `updated_at` + 正文哈希增量入库。短于 16 字符或少于 3 个词的碎片章节直接丢弃（它们会退化向量污染检索）。
3. **剪枝** —— 画师/角色类页面的正文是链接列表，其章节从检索索引中剔除（页面保留供精确查询）；这一步保证语义检索不被 URL 文本淹没。
