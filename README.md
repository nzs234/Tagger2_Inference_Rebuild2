...
| Tag Wiki | 本地 e621 / Danbooru 标签百科镜像：按分类浏览高频标签、模糊搜索、查看中文摘要与关联标签。 |
...
| Token Budget | 使用冻结 Tokenizer 检查超长文本，并允许人工修改 NL。 | 可选阈值，资源按需下载 |
...
- **缺失标签在线翻译**：词库缺失的标签可发给在线模型批量翻译，结果持久化到用户词典，重启后离线可用。
...
- **命令行运维**：目录构建与发行前校验通过 `scripts/build_tag_wiki_catalog.py` 完成，不在浏览器内修改 Wiki 数据。
- **离线友好**：目录浏览、搜索、词条详情和已有摘要展示全部离线可用。
...
每个资源由 manifest（resource_id、category、SHA-256 指纹、大小、来源）定义，内容寻址文件存放在 `data/workflows/resources/<category>/<resource_id>.<fingerprint前16位>`。自 V1.10 起，发行包只包含 manifest 与小型数据表；**模型类大文件（分类快照、Tokenizer）在首次使用时自动下载**，下载后按指纹校验再投入使用。也可以用 `runtime\python.exe scripts\fetch_workflow_resources.py` 预取，或把文件手动放入对应目录以支持完全离线部署。托管位置固定在 [resources-v1 发行页](https://github.com/nzs234/Tagger2_Inference_Rebuild2/releases/tag/resources-v1)。

| 类别 | Resource ID | 分发方式 | SHA-256 |
| --- | --- | --- | --- |
| 分类快照（e621） | `classify-e621-20260812-v1` | 按需下载 | `eccfdfacf3bcf1611a9ee3561f54bb81e946122f582f1f421c5e90689f2db49f` |
| 分类快照（danbooru） | `classify-danbooru-20260902-v1` | 按需下载 | 见 manifest |
| 推荐替换索引 | `replace-e621-pass-drop-v2` | 随包 | `2e3c4af6cc93b7f2cc8e55e2eda024ee69942f08a3618b6c2f0dfe6d45991972` |
| Tokenizer | `tokenizer-qwen3-0-6b-tokenizer-v1` | 按需下载 | `aeb13307a71acd8fe81861d94ad54ab689df773318809eed3cbe794b4492dae4` |
...
# 默认打包基础运行时（模型类依赖由 setup.bat 首次安装）
```
...
4. 复制后端、前端、脚本、锁文件和允许发布的不可变资源（模型类资源仅发 manifest）。
...
- [Tag Wiki 模块说明](docs/tag_wiki.md)
...
- [V1.10.5 发布说明](docs/V1.10.5_RELEASE_NOTES.md)
- [V1.10.4 发布说明](docs/V1.10.4_RELEASE_NOTES.md)
- [V1.10.3 发布说明](docs/V1.10.3_RELEASE_NOTES.md)
- [V1.10.2 发布说明](docs/V1.10.2_RELEASE_NOTES.md)
